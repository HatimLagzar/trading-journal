...
export default function BacktestingTradeChartView({
  trade,
  sessionName,
  systemLabel,
  onClose,
  ...
  const config = useMemo(() => {
    const symbol = `${normalizeBaseAsset(trade.asset)}USDT`
  ...
  }, [contextMultiplier, selectedInterval, trade.asset, trade.trade_date, trade.trade_time])
  ...
  useEffect(() => {
    ...
    const targetPriceLine = trade.target_price !== null
      ? series.createPriceLine({
    ...
    chart.timeScale().setVisibleLogicalRange({
      from: markerIndex - baseHalfBars,
      to: markerIndex + baseHalfBars,
    })
    ...
    return () => {
      ...
      setEntryLineX(null)
      if (entryPriceLine) series.removePriceLine(entryPriceLine)
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-sm sm:grid-cols-3">
          ...
          />
          <Detail label="Entry" value={formatOptionalPrice(trade.entry_price)} />
          <Detail label="Stop" value={formatOptionalPrice(trade.stop_loss)} />
          <Detail label="Target" value={formatOptionalPrice(trade.target_price)} />
          <Detail
            label="Outcome (R)"
            value={`${trade.outcome_r >= 0 ? '+' : ''}${trade.outcome_r.toFixed(2)}R`}
            valueClassName={trade.outcome_r >= 0
...
function readStoredTimeframe(): string {
  ...
  return TIMEFRAME_OPTIONS.some((option) => option.value === saved)
    ? saved
    : DEFAULT_TIMEFRAME
}
...
function getTradeTimestampMs(tradeDate: string, tradeTime: string | null): number {
  ...
  const utcDate = new Date(`${tradeDate}T${normalizedTime || '00:00:00'}Z`)
