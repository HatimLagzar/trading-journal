  ...
  return (
    <form onSubmit={handleImport} className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button type="button" onClick={onClose} className={isDark ? 'text-slate-400 hover:text-slate-200' : 'text-gray-500 hover:text-gray-700'}>
          ✕
        </button>
      </div>
      ...
      <div className={`space-y-3 rounded-lg border p-3 ${isDark ? 'border-white/10 bg-white/[0.03]' : 'bg-slate-50'}`}>
        <div>
          ...
          <select
            ...
            className={`w-full rounded-lg border px-3 py-2 ${isDark ? 'border-white/15 bg-slate-950 text-slate-100' : 'bg-white'}`}
          >
        ...
        <div className={`space-y-1 text-xs ${isDark ? 'text-slate-400' : 'text-gray-600'}`}>
              ...
              <table className="min-w-full text-xs">
                ...
                <tbody>
                  ...
                  {previewRows.length === 0 && (
                    <tr>
                      <td colSpan={Math.max(maxColumns, 1)} className={`px-2 py-3 text-center ${isDark ? 'text-slate-500' : 'text-gray-500'}`}>
...
function parseNumberValue(value: string): number | null {
  ...
  const cleaned = unwrapped
    .replace(/\b(?:USD|USDT|USDC|EUR|GBP|AUD|CAD|JPY|CHF|NZD|BTC|ETH)\b/gi, '')
    .replace(/[$€£¥₿₹,\s]/g, '')
    .replace(/(?!^)[+-]/g, '')
...
function normalizeImportedOutcome({
  ...
  const treatAsSigned =
    mode === 'signed' || realisedPnlValue !== null || sameSplitColumn
